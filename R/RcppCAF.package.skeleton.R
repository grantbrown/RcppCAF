## RcppCAF.package.skeleton: makes a skeleton for a package that wants to use CAF via Rcpp
##
## Based on RcppEigen.package.skeleton, 
## Copyright (C) 2011 - 2015  Douglas Bates, Dirk Eddelbuettel and Romain Francois
## Changes Copyright (C) 2015 Grant Brown
##
## This file is part of RcppCAF, and as such is covered by the GPL v3 license

RcppCAF.package.skeleton <- function(name= "anRpackage", list = character(),
    ...
    if (file.exists(DESCRIPTION)) {
        x <- cbind(read.dcf(DESCRIPTION), 
                   "Imports" = sprintf("Rcpp (>= %s), RcppEigen (>= %s), RcppCAF (>= %s)", 
                   packageDescription("Rcpp")[["Version"]], 
                   packageDescription("RcppEigen")[["Version"]], 
                   packageDescription("RcppCAF")[["Version"]]), 
                   "LinkingTo" = "Rcpp, RcppEigen, RcppCAF")
        write.dcf(x, file = DESCRIPTION)
        message(" >> added Imports: Rcpp, RcppEigen, RcppCAF")
        message(" >> added LinkingTo: Rcpp, RcppEigen, RcppCAF")
    }
    ...
    skeleton <- system.file("skeleton", package = "RcppCAF")
    Makevars <- file.path(src, "Makevars")
    if (!file.exists(Makevars)) {
        file.copy(file.path(skeleton, "Makevars"), Makevars)
        message(" >> added Makevars file with RcppCAF settings")
    }
	
    Makevars.win <- file.path(src, "Makevars.win")
    if (!file.exists(Makevars.win)) {
        file.copy(file.path(skeleton, "Makevars.win"), Makevars.win)
        message(" >> added Makevars.win file with RcppCAF settings")
    }
		
    if (example_code) {
        file.copy(file.path(skeleton, "rcppcaf_hello_world.cpp"), src)
        message(" >> added example src file using CAF classes")
        file.copy(file.path(skeleton, "rcppcaf_hello_world.Rd"), man)
        message(" >> added example Rd file for using CAF classes")
